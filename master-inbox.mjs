...
// `id` is optional and is the QA contract: the scripted employee tests and
// the destructive-control guard list address these buttons by id, so a rebuild
// that drops the ids silently disarms that guard.
const btn = (label, className, action, id) => { const node = el('button', className, label); node.type = 'button'; if (id) node.id = id; node.addEventListener('click', action); return node; };
...
const state = { route: normalizedRoute({ ...savedScope, ...Object.fromEntries(new URLSearchParams(location.search)), ...ROUTE.parseRoute(String(location.hash || '').replace(/^#/, '')) }), rows: [], boxes: [], selected: new Set(), thread: null, cursor: null, hasMore: false, coverage: null, parsed: null, loaded: false, loading: false, restorePages: 1, currentScreen: '', screenSequence: 0, listScroll: 0, listFocus: null, refreshAt: null, actionBusy: false };
...
function readToolbar(thread) {
  ...
  if (thread.recordType === 'draft') bar.append(btn('Edit draft', 'compose-button', () => composer.open('edit'), 'editDraft'));
  else if (thread.messages?.at(-1)?.direction === 'draft') bar.append(btn('Continue draft', 'compose-button', () => composer.open('continue-draft'), 'continueDraft'));
  if (thread.recordType === 'send' && ['held', 'scheduled'].includes(thread.sendState)) bar.append(btn('Cancel send', 'button', () => cancelSend(thread.id), 'cancelScheduled'));
  if (targets.length) bar.append(btn(['spam', 'trash'].includes(state.route.folder) ? 'Restore' : 'Archive', 'button', () => action(['spam', 'trash'].includes(state.route.folder) ? 'restore' : 'archive'), 'archiveAction'));
  const more = el('details', 'toolbar-more'); more.append(el('summary', '', 'More')); const menu = el('div'); if (normal) menu.append(btn('Forward…', '', () => { more.open = false; composer.open('forward'); }, 'forward')); if (targets.length) for (const [label, actionName] of [['Mark unread', 'unread'], ['Star', 'star'], ['Move to spam', 'spam'], ['Move to trash', 'trash']]) menu.append(btn(label, '', () => { more.open = false; action(actionName); })); if (!thread.recordType) { menu.append(btn('Add a tag…', '', () => { more.open = false; organize('tag'); }), btn('Snooze…', '', () => { more.open = false; organize('snooze'); })); } menu.append(btn('Copy conversation link', '', () => { const url = new URL(RaydarNav.href(routeAddress(state.route)), location.origin).href; navigator.clipboard.writeText(url).then(() => toast('Conversation link copied.')).catch(() => toast('Copy the conversation URL from your browser.')); }), btn('Print', '', () => window.print(), 'print')); more.append(menu); bar.append(more); return bar;
}
