...
// worker's dispatch loop. All three had every write gate defaulted closed
// since launch (never armed — see docs-site's paraai-reply-actioning.md,
// paraai-expired-actioning.md, paraai-curated-interest.md, all
// `status: deprecated`), so nothing that ever wrote to Paraform stopped
// writing. The kept lanes — outreach (this worker's own tick), the human-call
// pipeline (folded into runAutoTick/auto.mjs, untouched by this change) and
// submission-notify (a separate, already-retired endpoint this worker never
// called) — are asserted here to still dispatch exactly as before.
const source = readFileSync(
  new URL("../api/paraai/worker.mjs", import.meta.url),
  "utf8",
);

test("the three never-armed sub-lanes are gone from the dispatch loop", () => {
  assert.doesNotMatch(source, /runReplyTick\(\)/);
  assert.doesNotMatch(source, /runExpiredTick\(\)/);
  assert.doesNotMatch(source, /runInterestTick\(\)/);
  assert.doesNotMatch(source, /replyHealth\(\)/);
  assert.doesNotMatch(source, /expiredHealth\(\)/);
  ...
  assert.doesNotMatch(source, /from ".\/_lib\/expired\.mjs"/);
