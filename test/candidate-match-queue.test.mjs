...
test("Candidates replaces Needs Action with the fail-closed Match Queue", () => {
  ...
  assert.match(dashboard, /paraai-status\?outcomes=1/);
  assert.match(dashboard, /if\(attempt<2\)/);
});
...
test("outcome-only retry avoids another full CRM scan", async () => {
  let crmReads = 0;
  let jobReads = 0;
  const handler = createParaAIStatusHandler({
    corsImpl: () => false,
    ...
    loadSnapshot: async () => {
      crmReads++;
      throw new Error("CRM should not be read");
    },
    loadJobs: async () => {
      jobReads++;
      return [];
  ...
  const res = responseRecorder();
  await handler({ method: "GET", query: { outcomes: "1" }, headers: {} }, res);

  assert.equal(res.statusCode, 200);
  assert.equal(res.body.outcomeVerification.complete, true);
  assert.equal(res.body.outcomes.length, 1);
  assert.equal(res.body.outcomes[0].candidateUserId, "candidate-a");
  assert.equal(res.body.outcomes[0].verifiedOutcome, "Sent List");
