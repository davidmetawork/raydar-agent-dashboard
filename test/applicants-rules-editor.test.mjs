...
test("a missing-ID company seed renders, previews, saves and manually evaluates with optional same-job conditions", async (t) => {
  const state = createFixtureState();
  const server = createFixtureServer({ state });
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  t.after(() => new Promise((resolve) => server.close(resolve)));
  ...
  vm.runInNewContext(await readFile(new URL("../applicants-rule-facts.js", import.meta.url), "utf8"), context);
  const chooser = context.RaydarRuleFacts;
  const row = chooser.profileSources(candidate.profileKey, candidate, profile).find((item) => item.kind === "experience" && item.source === "paraform");
  assert.equal(row.record.companyId, null);
  const rule = { ...chooser.createSeed(row, ["experience-company-name"]), id: "fixture-company-rule", action: "pass", state: "live", scope: { roleIds: [] } };
  const catalog = await fetch(`${base}/api/applicants/rules`).then((response) => response.json());
  const { testApi } = await loadRulesUi(async () => response({ ok: true }));
  testApi.state.catalog = catalog.catalog;
  const input = testApi.valueControl(rule.conditions[0], 0);
  assert.match(input, /aria-label="Company name"/);
  assert.match(input, /value="Figma"/);
  assert.match(input, /ignoring case and spacing/);
  const preview = await post("/api/applicants/rules", { op: "preview", rule, ...fence });
  assert.equal(preview.matched, 1);
  assert.equal(preview.samples[0].name, candidate.name);
  assert.equal(preview.samples[0].evidence[0].source, "paraform");
  const normalized = await post("/api/applicants/rules", { op: "preview", ...fence, rule: { ...rule, conditions: [{ field: "job.companyName", op: "equals", value: " FIGMA " }] } });
  assert.equal(normalized.matched, 1);
  const current = await post("/api/applicants/rules", { op: "preview", ...fence, rule: { ...rule, conditions: [...rule.conditions, { field: "job.current", op: "is", value: true }] } });
  assert.equal(current.matched, 0);
  const refined = { ...rule, ...chooser.createSeed(row, ["experience-company-name", "experience-title", "experience-current"]) };
  const refinedPreview = await post("/api/applicants/rules", { op: "preview", rule: refined, ...fence });
  assert.equal(refinedPreview.matched, 1);
  assert.deepEqual(state.decisions, {});
  const saved = await post("/api/applicants/rules", { op: "save", rev: state.rev, rule: refined });
  assert.equal(saved.ok, true);
  assert.deepEqual(state.decisions, {}, "saving does not run rules");
  state.rules = [state.rules.find((item) => item.id === rule.id)];
  const run = await post("/api/applicants/rules-tick", fence);
  assert.equal(run.decided, refinedPreview.matched);
  assert.deepEqual(Object.keys(state.decisions), [candidate.key]);
