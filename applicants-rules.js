...
(function () {
  ...
  function previewHtml() {
    ...
    return '<div class="preview" role="status"><div class="preview-heading"><span class="n">' + Number(preview.matched).toLocaleString() + '</span><span>matching applicant' + (preview.matched === 1 ? '' : 's') + '</span></div>' +
      ...
      (factsPending ? '<div class="sub">' + factsPending.toLocaleString() + ' applicant' + (factsPending === 1 ? ' is' : 's are') + ' waiting for verified profile facts; this match count is incomplete.</div>' : '') +
    ...
    try {
      ...
      if (preview.paged && preview.operationId) {
        ...
        preview = {
          ...
          skipped: { unavailable: Number(completed.operation.skippedCount || 0) },
