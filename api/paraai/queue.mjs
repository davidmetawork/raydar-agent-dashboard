...
import { jobReviewReasons, reviewActionFor } from "./_lib/review.mjs";
import { listJobs, storeConfigured } from "./_lib/store.mjs";

export const config = { maxDuration: 30 };

function enrichReviewJob(job) {
  const reviewReasons = jobReviewReasons(job);
  return {
    ...job,
    reviewReasons,
    reviewAction: reviewActionFor(job, reviewReasons),
  };
}

export function aggregateReviewJobs(jobs = []) {
  const groups = new Map();
  for (const job of jobs.map(enrichReviewJob)) {
    const key = String(job?.identity?.candidateUserId || job?.id || "");
    if (!groups.has(key)) {
      groups.set(key, {
        ...job,
    ...
    const reviewReasons = jobReviewReasons({
      reviewReasons: [...current.reviewReasons, ...job.reviewReasons],
    });
    groups.set(key, {
      ...current,
      relatedJobIds: [...current.relatedJobIds, job.id],
      reviewReasons,
      reviewAction: reviewActionFor(current, reviewReasons),
    });
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const groups = {
      ...
      needsReview: aggregateReviewJobs(jobs.filter((job) => job.state === "needs_review")),
