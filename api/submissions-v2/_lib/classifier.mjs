...
function prompt(event) {
  return JSON.stringify({
    ...
    rules: [
      ...
      "A plural or collective positive that refers to the offered set, such as \"these\", \"those\", \"either of these\", or \"the roles\", is interested on every offered role even when the words both or all are absent; the phrase must be plural and must refer to the roles offered, not to a future conversation.",
      "Preference wording such as particularly, especially, or my favourites is emphasis rather than exclusion when the same reply also carries a separate set-wide positive clause; without that separate clause, apply the reply only to the roles the candidate named.",
      "A stated exception inside a blanket positive, such as interest in all of them except one named company, is interested on the remaining offered roles and not_interested on the excluded role, quoting the exception clause.",
      "A blanket decline of the offered set is not_interested on every offered role, unless the sentence scopes itself to one named company or role.",
      "Clear unconditional interest also remains interested when the only other content is a process question about next steps, timing, or an introduction, or a report that a job description or application link did not open.",
      "Politeness, gratitude, or a promise to take a look is never interest on its own; without a named role or a stated intent it is needs_review.",
      "A pronoun or demonstrative such as this role, it, or this one is a decisive reference when exactly one role was offered; when more than one role was offered it identifies no role and is role_unclear.",
      "An exact role URL pasted by the candidate identifies that role's scope but is never proof of interest; a decisive label still needs separate intent language in the candidate's own words.",
