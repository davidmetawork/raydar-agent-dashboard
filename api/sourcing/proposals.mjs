...
import { getRoleState, getRun, saveRoleState, saveRun, storeConfigured } from "./_lib/store.mjs";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const roleState = await getRoleState(run.roleId);
    ...
    const savedRun = await saveRun({ ...run, proposalDecisions: [...(run.proposalDecisions || []), decision] }, run.revision);
    await saveRoleState(run.roleId, {
      ...roleState,
      activeRubricVersionId: revision.id,
      rubricVersions: [...roleState.rubricVersions, revision],
    });
    return res.status(200).json({ ok: true, run: savedRun, rubricVersion: revision });
  } catch (error) {
