import { cors, requireAuth, hasCookie, buildPlan, ensureRoleSequence, enrollIntoCampaign, addToProject, setCandidateEmail, setLeadEmail, ccuIndex, enrolledElsewhereSet, archiveImportSet, createDelayProject, resolveSequenceCandidate, sequencesUnmatchedLinkedinCreateGranted } from "./_lib/core.mjs";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Delayed mode: resolve existing candidates now and, only when the
    // separately named create grant is enabled, create unmatched candidates. File
    // resolved identities into dated "⏳SeqDelay" projects and stop. The daily
    ...
    if (delayDays > 0) {
      ...
      return res.status(200).json({
        ...
        unmatchedCreateGranted: sequencesUnmatchedLinkedinCreateGranted(),
    ...
    for (const g of plan.groups) {
      ...
      if (prot) { protectedBlocked.push({ role: g.title, recruiter: prot.displayName, candidates: (g.rows || []).length }); continue; }
      try {
        // 1) Resolve each row to a candidate_user_id. Unmatched identities are parked
        //    unless the separately named create grant is enabled. Track isNew so we
        //    only booked-check pre-existing people.
        const resolved = await mapPool(
          g.rows || [],
          4,
          (row) => resolveSequenceCandidate(row),
